...
export class UnitEditor {
  ...
  private appendGunRow(gun?: Gun): void {
    ...
    const ammoPerInput = row.querySelector<HTMLInputElement>('input[data-field="ammoPerSoldier"]');
    const totalAmmoInput = row.querySelector<HTMLInputElement>('input[data-field="totalAmmo"]');
    const recalcTotalAmmo = () => {
      if (!countInput || !ammoPerInput || !totalAmmoInput) return;
      const count = Math.max(1, parseInt(countInput.value || "1", 10));
      const per = Math.max(0, parseInt(ammoPerInput.value || "0", 10));
      countInput.value = count.toString();
      ammoPerInput.value = per.toString();
      totalAmmoInput.value = (count * per).toString();
    };
    ...
    trajRow.innerHTML = `<span class="label">Trajectories</span>`;
    const trajWrap = document.createElement("div");
    trajWrap.className = "chip-wrap";
    ["Direct", "Indirect", "LAW", "RR", "ATGM", "MANPADS"].forEach((option) => {
      const btn = document.createElement("button");
      btn.type = "button";
      btn.className = "chip-button traj-chip";
      btn.dataset.traj = option;
      btn.textContent = option;
      if ((gun?.trajectories || []).map((t) => t.toLowerCase()).includes(option.toLowerCase())) {
        btn.classList.add("active");
      }
      btn.addEventListener("click", () => btn.classList.toggle("active"));
      trajWrap.appendChild(btn);
    });
    trajRow.appendChild(trajWrap);

    const datalistCache = new Map<string, HTMLDataListElement>();
    const ensureAmmoDatalist = (key: string) => {
      const normalized = key || "generic";
      if (!datalistCache.has(normalized)) {
        const dl = document.createElement("datalist");
        dl.id = `ammo-dl-${normalized}-${Math.random().toString(36).slice(2, 7)}`;
        datalistCache.set(normalized, dl);
        row.appendChild(dl);
      }
      const dl = datalistCache.get(normalized)!;
      dl.innerHTML = "";
      const options = this.ammoLibraryByCaliber.get(normalized) || this.ammoLibraryByCaliber.get("generic") || [];
      options.forEach((ammo) => {
        if (!ammo.name) return;
    ...
    const appendAmmoRow = (ammo?: GunAmmo) => {
      ...
      const nameField = ammoRow.querySelector<HTMLInputElement>('input[data-ammo-field="name"]');
      const bindDatalist = () => {
        if (!nameField) return;
        const dl = ensureAmmoDatalist(getCaliberKey());
      ...
      const applyBallistics = () => {
        ...
        const trajectories = getTrajectories().map((t) => t.toUpperCase());
        const heavy = ["LAW", "RR", "ATGM", "MANPADS"].some((tag) => trajectories.includes(tag));
        let velocity = heavy ? 280 + barrel * 8 : 600 + barrel * 25;
        if (grain > 0) velocity -= grain * (heavy ? 0.3 : 0.8);
        fpsInput.value = Math.max(150, Math.round(velocity)).toString();
      };
      grainInput?.addEventListener("input", applyBallistics);
      barrelInput?.addEventListener("input", () => {
        applyBallistics();
        bindDatalist();
      });
      trajWrap.addEventListener("click", applyBallistics);
      applyBallistics();
    };
    ...
    row.append(trajRow, fireSection, ammoSection);
  ...
  private collectGunRows(): Gun[] {
    ...
    rows.forEach((row) => {
      ...
      const trajectories = Array.from(row.querySelectorAll<HTMLButtonElement>(".traj-chip"))
        .filter((chip) => chip.classList.contains("active"))
        .map((chip) => chip.dataset.traj || "")
        .filter(Boolean);
      if (trajectories.length) gun.trajectories = trajectories;
